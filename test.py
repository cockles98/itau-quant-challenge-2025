...
price_frame = pd.DataFrame(
    {
        'AAA': 100 * np.exp(np.cumsum(0.0002 + 0.01 * np.random.randn(len(price_index)))),
        'BBB': 120 * np.exp(np.cumsum(-0.0001 + 0.012 * np.random.randn(len(price_index)))),
    },
    index=price_index,
)
...
long_prices = pd.DataFrame(
    {
        'AAA': 100 * np.exp(np.cumsum(0.0003 + 0.012 * np.random.randn(len(long_index)))),
        'BBB': 90 * np.exp(np.cumsum(0.0001 + 0.011 * np.random.randn(len(long_index)))),
        'CCC': 110 * np.exp(np.cumsum(-0.0002 + 0.013 * np.random.randn(len(long_index)))),
    },
    index=long_index,
)
...
scaled_vol = np.sqrt(float(scaled_weights.loc[active_assets] @ risk_cov.loc[active_assets, active_assets] @ scaled_weights.loc[active_assets]))
...
capped_weights = risk_controls.apply_caps(weights, max_asset=0.35, max_cluster=0.55, clusters=clusters)
...
ks_trigger = risk_controls.kill_switch(equity, vol_series, vol_target=0.10, mdd_lookback=60, mdd_thres=-0.15, vol_mult=1.5)
...
metric_returns = pd.Series(np.random.normal(0.0005, 0.01, len(metric_index)), index=metric_index)
...
curves_path = plot_equity_curves({'Strategy': metric_equity, 'Benchmark': metric_equity * 0.98})
print('Equity plot saved to:', curves_path)

results_table = table_kpis({
    'Strategy': {'equity_curve': metric_equity, 'returns': metric_returns, 'weights': weights_df},
    'Benchmark': {'equity_curve': metric_equity * 0.98, 'returns': metric_returns * 0.8},
})
print('KPI table:\n', results_table)
...
for asset in wf_assets:
    ...
    asset_df = pd.DataFrame({'date': wf_dates, 'asset': asset, 'close': path, 'volume': volume})
...
for asset in assets_cv:
    ...
    cv_records.append(pd.DataFrame({'date': cv_dates, 'asset': asset, 'close': price_path, 'volume': volume_series}))
...
best_params, grid_results = tune_params(cfg_purged, cv_panel, param_grid, n_splits=2, embargo_days=5)
